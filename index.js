...
app.post('/api/sample', async (req, res) => {
  ...
  try {
    ...
    // 2️⃣ Klaviyo toevoegen aan lijst via Add Profiles to List
    const klaviyoPayload = {
      data: [
        { type: "profile", id: email }
      ]
    };
    ...
    if (!klaviyoResponse.ok) {
      console.warn("⚠️ Klaviyo fout:", klaviyoData);
      // We geven de QLS-order succes terug, ook al faalt Klaviyo
      return res.status(200).json({
        message: "✅ Sample succesvol aangevraagd! ⚠️ Maar Klaviyo toevoegen faalde.",
        qlsOrderId: qlsData.data?.id || null,
        klaviyoError: klaviyoData
      });
    }
